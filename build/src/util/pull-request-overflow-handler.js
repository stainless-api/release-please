...
class FilePullRequestOverflowHandler {
     ...
     *   the full content.
     */
    async handleOverflow(pullRequest, baseBranch, maxSize = MAX_ISSUE_BODY_SIZE) {
        const notes = pullRequest.body.toString();
        if (notes.length > maxSize) {
            const notesBranchName = `${pullRequest.headRefName}--release-notes`;
            const url = await this.github.createFileOnNewBranch(RELEASE_NOTES_FILENAME, notes, notesBranchName, baseBranch);
