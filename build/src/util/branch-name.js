...
class BranchName {
    ...
    static ofTargetBranch(targetBranch, changesBranch) {
        const changes = targetBranch === changesBranch ? '' : `--changes--${changesBranch}`;
        return new DefaultBranchName(`${RELEASE_PLEASE}--branches--${targetBranch}${changes}`);
    }
    static ofComponentTargetBranch(component, targetBranch, changesBranch) {
        const changes = targetBranch === changesBranch ? '' : `--changes--${changesBranch}`;
        return new ComponentBranchName(`${RELEASE_PLEASE}--branches--${targetBranch}${changes}--components--${component}`);
    }
    static ofGroupTargetBranch(group, targetBranch, changesBranch) {
        const changes = targetBranch === changesBranch ? '' : `--changes--${changesBranch}`;
        return new GroupBranchName(`${RELEASE_PLEASE}--branches--${targetBranch}${changes}--groups--${safeBranchName(group)}`);
    }
...
class DefaultBranchName extends BranchName {
    ...
    toString() {
        const changes = !this.changesBranch || this.targetBranch === this.changesBranch
            ? ''
