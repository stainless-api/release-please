...
function generateMatchPattern(pullRequestTitlePattern, logger = logger_1.logger) {
    ...
    return new RegExp(`^${(pullRequestTitlePattern || DEFAULT_PR_TITLE_PATTERN)
        ...
        .replace('(', '\\(')
        .replace(')', '\\)')
        .replace('${scope}', '(\\((?<changesBranch>[\\w-./]+ => )?(?<branch>[\\w-./]+)\\))?')
...
class PullRequestTitle {
    ...
    static ofTargetBranchVersion(targetBranch, changesBranch, version, pullRequestTitlePattern) {
        return new PullRequestTitle({
            version,
            targetBranch,
            changesBranch,
    ...
    static ofComponentTargetBranchVersion(component, targetBranch, changesBranch, version, pullRequestTitlePattern) {
        return new PullRequestTitle({
            version,
            component,
            targetBranch,
    ...
    static ofTargetBranch(targetBranch, changesBranch, pullRequestTitlePattern) {
        return new PullRequestTitle({
            targetBranch,
            changesBranch,
            pullRequestTitlePattern,
