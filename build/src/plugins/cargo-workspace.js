...
class CargoWorkspace extends workspace_1.WorkspacePlugin {
    async buildAllPackages(candidates) {
        var _a, _b, _c, _d;
        const cargoManifestContent = await this.github.getFileContentsOnBranch('Cargo.toml', this.changesBranch);
        ...
        const members = (await Promise.all(cargoManifest.workspace.members.map(member => this.github.findFilesByGlobAndRef(member, this.changesBranch)))).flat();
        ...
        for (const path of members) {
            ...
            const manifestContent = ((_b = candidate === null || candidate === void 0 ? void 0 : candidate.pullRequest.updates.find(update => update.path === manifestPath)) === null || _b === void 0 ? void 0 : _b.cachedFileContents) ||
                (await this.github.getFileContentsOnBranch(manifestPath, this.changesBranch));
    ...
    newCandidate(pkg, updatedVersions) {
        ...
        const dependencyNotes = getChangelogDepsNotes(originalManifest, updatedManifest);
        const pullRequest = {
            title: pull_request_title_1.PullRequestTitle.ofTargetBranch(this.targetBranch, this.changesBranch),
