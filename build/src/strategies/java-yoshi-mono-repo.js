...
class JavaYoshiMonoRepo extends java_1.Java {
    ...
    async getVersionsContent() {
        if (!this.versionsContent) {
            try {
                this.versionsContent = await this.github.getFileContentsOnBranch(this.addPath('versions.txt'), this.changesBranch);
            }
            catch (err) {
    ...
    async buildUpdates(options) {
        ...
        const pomFilesSearch = this.github.findFilesByFilenameAndRef('pom.xml', this.changesBranch, this.path);
        const buildFilesSearch = this.github.findFilesByFilenameAndRef('build.gradle', this.changesBranch, this.path);
        const dependenciesSearch = this.github.findFilesByFilenameAndRef('dependencies.properties', this.changesBranch, this.path);
        const readmeFilesSearch = this.github.findFilesByFilenameAndRef('README.md', this.changesBranch, this.path);
    ...
    async hasChangelogJson() {
        try {
            const content = await this.github.getFileContentsOnBranch('changelog.json', this.changesBranch);
            return !!content;
        }
        catch (e) {
    ...
    async getRepoMetadata(path) {
        try {
            const content = await this.github.getFileContentsOnBranch(this.addPath(`${path}/.repo-metadata.json`), this.changesBranch);
