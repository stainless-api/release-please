...
class KRMBlueprint extends base_1.BaseStrategy {
    async buildUpdates(options) {
        ...
        const yamlPaths = await this.github.findFilesByExtensionAndRef('yaml', this.changesBranch, this.path);
        for (const yamlPath of yamlPaths) {
            const contents = await this.github.getFileContentsOnBranch(this.addPath(yamlPath), this.changesBranch);
