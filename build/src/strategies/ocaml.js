...
class OCaml extends base_1.BaseStrategy {
    async buildUpdates(options) {
        ...
        const jsonPaths = await this.github.findFilesByExtensionAndRef('json', this.changesBranch, this.path);
        for (const path of jsonPaths) {
            if (notEsyLock(path)) {
                const contents = await this.github.getFileContentsOnBranch(this.addPath(path), this.changesBranch);
        ...
        const opamPaths = await this.github.findFilesByExtensionAndRef('opam', this.changesBranch, this.path);
        ...
        const opamLockedPaths = await this.github.findFilesByExtensionAndRef('opam.locked', this.changesBranch, this.path);
