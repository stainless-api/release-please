...
class Python extends base_1.BaseStrategy {
    ...
    async buildUpdates(options) {
        ...
        const versionPyFilesSearch = this.github.findFilesByFilenameAndRef('version.py', this.changesBranch, this.path);
    ...
    async getPyProject(path) {
        try {
            const content = await this.github.getFileContentsOnBranch(path, this.changesBranch);
    ...
    async getSetupPyContents() {
        try {
            return (await this.github.getFileContentsOnBranch(this.addPath('setup.py'), this.changesBranch)).parsedContent;
        }
        catch (e) {
