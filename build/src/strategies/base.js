...
`, existingPullRequest.number);
    ...
    async extraFilePaths(extraFile) {
        ...
        if (extraFile.path.startsWith('/')) {
            ...
            return (await this.github.findFilesByGlobAndRef(extraFile.path.slice(1), this.changesBranch)).map(file => `/${file}`);
        }
        else if (this.path === manifest_1.ROOT_PROJECT_PATH) {
            // root component, ignore path prefix
            return this.github.findFilesByGlobAndRef(extraFile.path, this.changesBranch);
        }
        else {
            // glob is relative to current path
            return this.github.findFilesByGlobAndRef(extraFile.path, this.changesBranch, this.path);
        }
    }
