...
class TerraformModule extends base_1.BaseStrategy {
    async buildUpdates(options) {
        ...
        const readmeFiles = await Promise.all([
            this.github.findFilesByFilenameAndRef('readme.md', this.changesBranch, this.path),
            this.github.findFilesByFilenameAndRef('README.md', this.changesBranch, this.path),
        ]).then(([v, vt]) => {
            return v.concat(vt);
        });
        ...
        const versionFiles = await Promise.all([
            this.github.findFilesByFilenameAndRef('versions.tf', this.changesBranch, this.path),
            this.github.findFilesByFilenameAndRef('versions.tf.tmpl', this.changesBranch, this.path),
        ]).then(([v, vt]) => {
            return v.concat(vt);
        });
