...
class PHPYoshi extends base_1.BaseStrategy {
    ...
    async buildReleasePullRequest({ commits, labels = [], latestRelease, draft, }) {
        ...
        for (const directory of topLevelDirectories) {
            try {
                const contents = await this.github.getFileContentsOnBranch(this.addPath(`${directory}/VERSION`), this.changesBranch);
                const version = version_1.Version.parse(contents.parsedContent);
                const composer = await this.github.getFileJson(this.addPath(`${directory}/composer.json`), this.changesBranch);
        ...
        const pullRequestTitle = pull_request_title_1.PullRequestTitle.ofComponentTargetBranchVersion(component || '', this.targetBranch, this.changesBranch, newVersion);
        const branchName = component
            ? branch_name_1.BranchName.ofComponentTargetBranch(component, this.targetBranch, this.changesBranch)
