...
class GitHub {
    constructor(options) {
        ...
        this.graphqlRequest = wrapAsync(async (opts, options) => {
            var _a;
            let maxRetries = (_a = options === null || options === void 0 ? void 0 : options.maxRetries) !== null && _a !== void 0 ? _a : this.graphqlRetries;
            let seconds = 1;
            while (maxRetries >= 0) {
                try {
        ...
        this.updatePullRequest = wrapAsync(async (number, releasePullRequest, baseBranch, refBranch, options) => {
            ...
            if (currentPr.data.state !== 'open') {
                this.logger.warn(`Pull request #${number} is no longer open (state: ${currentPr.data.state}) - skipping update`);
        ...
        this.createRelease = wrapAsync(async (release, options = {}) => {
            ...
            if ((0, errors_1.isOctokitRequestError)(e) &&
                e.status === 422 &&
                errors_1.GitHubAPIError.parseErrors(e).some(error => error.code === 'already_exists')) {
        ...
        this.graphqlRetries = (_c = options.graphqlRetries) !== null && _c !== void 0 ? _c : 5;
    ...
    static async create(options) {
        ...
        const OctokitWithPlugins = rest_1.Octokit.plugin(plugin_retry_1.retry, plugin_throttling_1.throttling);
        const logger = (_c = options.logger) !== null && _c !== void 0 ? _c : logger_2.logger;
        const throttlingRetries = (_d = options.throttlingRetries) !== null && _d !== void 0 ? _d : 0;
        const apis = (_e = options.octokitAPIs) !== null && _e !== void 0 ? _e : {
            octokit: new OctokitWithPlugins({
                ...
                throttle: {
                    ...
                    onRateLimit: (retryAfter, options, octokit, retryCount) => {
                        ...
                        octokit.log.warn(`[octokit-throttling] Request quota exhausted for request ${method} ${url}`);
                        if (retryCount < throttlingRetries) {
                            octokit.log.info(`Attempt ${retryCount}: retrying after ${retryAfter} seconds`);
                            return true;
                        }
                        return undefined;
                    },
                    ...
                    onSecondaryRateLimit: (retryAfter, options, octokit, retryCount) => {
                        const method = 'method' in options ? options.method : 'UnknownMethod';
                        const url = 'url' in options ? options.url : 'UnknownUrl';
                        octokit.log.warn(`[octokit-throttling] SecondaryRateLimit reached for request ${method} ${url}`);
                        if (retryCount < throttlingRetries) {
                            octokit.log.info(`Attempt ${retryCount}: retrying after ${retryAfter} seconds`);
                            return true;
                        }
            ...
            request: request_1.request.defaults({
                baseUrl: apiUrl,
                headers: {
                    'user-agent': `@stainless-api/release-please/${((_g = options.version) === null || _g === void 0 ? void 0 : _g.toString()) || '0.0.0-dev'}`,
            ...
            graphql: graphql_1.graphql.defaults({
                ...
                headers: {
                    'user-agent': `@stainless-api/release-please/${((_h = options.version) === null || _h === void 0 ? void 0 : _h.toString()) || '0.0.0-dev'}`,
                    Authorization: `token ${options.token}`,
        ...
        const opts = {
            repository: {
                owner: options.owner,
                repo: options.repo,
                defaultBranch: (_j = options.defaultBranch) !== null && _j !== void 0 ? _j : (await GitHub.defaultBranch(options.owner, options.repo, apis.octokit)),
            },
            octokitAPIs: apis,
            logger,
    ...
    async isBranchSyncedWithPullRequestCommits(branchName, pullRequest) {
        ...
        const latestPRCommit = lastPageOfPrCommits.data[lastPageOfPrCommits.data.length - 1];
    ...
    async isBranchASyncedWithB(branchAName, branchBName) {
        ...
        if (comparison.data.status === 'diverged') {
            ...
            const exclusiveCommitsA = new Set(comparison.data.commits.map(commit => commit.sha));
            const exclusiveCommitsB = new Set((await this.octokit.repos.compareCommitsWithBasehead({
                owner: this.repository.owner,
                repo: this.repository.repo,
            ...
            const getChangedFiles = (files) => (files === null || files === void 0 ? void 0 : files.map(file => ({
                sha: file.sha,
            ...
            for (const shaA of exclusiveCommitsA) {
                ...
                for (const shaB of exclusiveCommitsB) {
                    ...
                    if (!filesBMap || !metaB) {
                        ...
                        filesBMap = new Map(getChangedFiles(commit.data.files).map(file => [file.sha, file]));
                    ...
                    foundInB =
                        metaA.message === metaB.message &&
                            filesA.every(fileA => {
                                const fileB = filesBMap === null || filesBMap === void 0 ? void 0 : filesBMap.get(fileA.sha);
                                if (!fileB)
                                    return false;
                ...
                if (!foundInB) {
                    this.logger.debug(`Commit ${shaA} from branch ${branchAName} not found in ${branchBName}`);
     ...
     */
    async waitForReleaseToBeListed({ tagName, id }) {
        for (let attempt = 0; attempt < 10; attempt++) {
            this.logger.debug(`Checking if release ${tagName} is listed on GitHub (attempt ${attempt + 1})...`);
     ...
     **/
    async waitForFileToBeUpToDateOnBranch({ branch, filePath, checkFileStatus, }) {
        const maxAttempts = 10;
        let notFoundError = undefined;
        for (let attempt = 0; attempt < maxAttempts; attempt++) {
            this.logger.debug(`Checking if file ${filePath} on branch ${branch} is up to date on GitHub (attempt ${attempt + 1})...`);
            ...
            try {
                ...
                if (upToDate) {
                    this.logger.debug(`File ${filePath} on branch ${branch} seems up to date on GitHub`);
                    return;
                }
            }
            catch (e) {
                ...
                if (e instanceof errors_1.FileNotFoundError) {
                    notFoundError = e;
                    this.logger.warn(`Failed to fetch ${filePath} on branch ${branch}`, notFoundError);
                }
                else {
                    throw e;
                }
        ...
        this.invalidateFileCache();
        this.logger.warn(`File ${filePath} on branch ${branch} is not up to date yet on GitHub`);
    }
    invalidateFileCache() {
        this.fileCache = new git_file_utils_1.RepositoryFileCache(this.octokit, this.repository);
    }
    async queryPullRequestId(pullRequestNumber) {
        var _a, _b;
        ...
        const id = (_b = (_a = response === null || response === void 0 ? void 0 : response.repository) === null || _a === void 0 ? void 0 : _a.pullRequest) === null || _b === void 0 ? void 0 : _b.id;
        if (id) {
            return id;
        }
        return undefined;
    }
    async mutatePullRequestEnableAutoMerge(pullRequestId, mergeMethod) {
