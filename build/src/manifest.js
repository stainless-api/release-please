...
class Manifest {
    ...
    constructor(github, targetBranch, repositoryConfig, releasedVersions, manifestOptions) {
        ...
        this.changesBranch = (manifestOptions === null || manifestOptions === void 0 ? void 0 : manifestOptions.changesBranch) || this.targetBranch;
        ...
        this.prereleaseLabels =
            (manifestOptions === null || manifestOptions === void 0 ? void 0 : manifestOptions.prereleaseLabels) || exports.DEFAULT_PRERELEASE_LABELS;
    ...
    static async fromConfig(github, targetBranch, config, manifestOptions, path = exports.ROOT_PROJECT_PATH) {
        ...
        const latestVersion = await latestReleaseVersion(github, (manifestOptions === null || manifestOptions === void 0 ? void 0 : manifestOptions.changesBranch) || targetBranch, version => isPublishedVersion(strategy, version), config, component, manifestOptions === null || manifestOptions === void 0 ? void 0 : manifestOptions.logger);
        if (latestVersion) {
     ...
     */
    async buildPullRequests(openPullRequests, snoozedPullRequests) {
        var _a;
        ...
        if (!this.separatePullRequests) {
            this.plugins.push(new merge_1.Merge(this.github, this.targetBranch, this.manifestPath, this.repositoryConfig, {
    ...
    async createPullRequests() {
        ...
        const candidatePullRequests = await this.buildPullRequests(openPullRequests, snoozedPullRequests);
    ...
    async registerLabels() {
        const repoLabels = new Set(await this.github.getLabels());
        const missingLabels = [
            ...this.labels,
            ...this.releaseLabels,
            ...this.prereleaseLabels,
    ...
    async createOrUpdatePullRequest(pullRequest, openPullRequests, snoozedPullRequests) {
        ...
        const body = await this.pullRequestOverflowHandler.handleOverflow(pullRequest, this.changesBranch);
    ...
    async maybeUpdateSnoozedPullRequest(snoozed, pullRequest) {
        ...
        const updatedPullRequest = await this.github.updatePullRequest(snoozed.number, pullRequest, this.targetBranch, this.changesBranch, {
            fork: this.fork,
            signoffUser: this.signoffUser,
            pullRequestOverflowHandler: this.pullRequestOverflowHandler,
        });
    ...
    async buildReleases() {
        ...
        for await (const pullRequest of generator) {
            for (const path in this.repositoryConfig) {
                ...
                const hasPrereleaseLabel = !!pullRequest.labels.find(label => this.prereleaseLabels.find(prereleaseLabel => prereleaseLabel === label));
     ...
     */
    async lockPullRequestsChangesBranch(pullRequests) {
        const lockedBranches = new Set();
        for (const pr of pullRequests) {
            const branchName = branch_name_1.BranchName.parse(pr.headBranchName);
            const refBranch = (branchName === null || branchName === void 0 ? void 0 : branchName.changesBranch) || (branchName === null || branchName === void 0 ? void 0 : branchName.targetBranch);
    ...
    async unlockPullRequestsChangesBranch(branchNames) {
        for (const branch of branchNames) {
            await this.github.unlockBranch(branch);
        }
    }
    async alignPullRequestsChangesBranch(pullRequests) {
        const errors = [];
        for (const pr of pullRequests) {
            try {
        ...
        if (errors.length > 0) {
            throw new errors_1.AggregateError(errors, 'Errors when aligning pull requests branches');
        }
    }
    async alignPullRequestChangesBranch(pr) {
        ...
        this.logger.info(`Aligning branches for PR #${pr.number}, changes branch ${branchName.changesBranch} to be aligned with ${this.targetBranch}`);
        let safeToRealign = false;
        try {
            this.logger.debug(`Checking if PR commits are in sync with '${branchName.changesBranch}'...`);
            if (await this.github.isBranchSyncedWithPullRequestCommits(branchName.changesBranch, pr)) {
                this.logger.debug('PR commits and changes branch in sync, safe to re-align');
                safeToRealign = true;
            }
        }
        catch (err) {
            ...
            if ((0, errors_1.isOctokitRequestError)(err) && err.status === 404) {
                this.logger.debug(`Could not compare commits from PR and '${branchName.changesBranch}' due to a branch or commit not found. Continue with the next check`);
            }
            else {
                throw err;
            }
        ...
        // anything
        if (!safeToRealign &&
            (await this.github.isBranchASyncedWithB(branchName.changesBranch, this.targetBranch))) {
            this.logger.debug(`Checking if ${branchName.changesBranch} is synced with ${this.targetBranch}...`);
            this.logger.debug('Branches already in sync, no need to re-align');
            return;
        }
        if (!safeToRealign) {
            throw new Error(`Branch '${branchName.changesBranch}' cannot be safely re-aligned with '${this.targetBranch}', and will likely result in git conflicts when the next release PR is created. Hint: compare branches '${pr.headBranchName}', '${branchName.changesBranch}', and '${this.targetBranch}' for inconsistencies`);
        }
        ...
        await this.github.waitForFileToBeUpToDateOnBranch({
            ...
            checkFileStatus: fileContent => {
                ...
                if (typeof val !== 'string') {
                    this.logger.error(`Value of manifest file ${this.manifestPath} at key '${path}' was not a string. value=${val}`);
                    return false;
                }
...
function extractReleaserConfig(config) {
    ...
    return {
        ...
        prereleaseLabels: (_c = config['prerelease-label']) === null || _c === void 0 ? void 0 : _c.split(','),
        extraLabels: (_d = config['extra-label']) === null || _d === void 0 ? void 0 : _d.split(','),
 ...
 */
async function latestReleaseVersion(github, branchToScan, releaseFilter, config, prefix, logger = logger_1.logger) {
    const branchPrefix = prefix
        ? prefix.endsWith('-')
            ? prefix.replace(/-$/, '')
            : prefix
        : undefined;
    logger.info(`Looking for latest release on branch: ${branchToScan} with prefix: ${prefix}`);
    ...
    for await (const release of releaseGenerator) {
        ...
        if (tagMatchesConfig(tagName, branchPrefix, config.includeComponentInTag)) {
            ...
            if (!commitShas.has(release.sha)) {
                logger.debug(`SHA not found in recent commits to branch '${branchToScan}', skipping`);
                continue;
            }
